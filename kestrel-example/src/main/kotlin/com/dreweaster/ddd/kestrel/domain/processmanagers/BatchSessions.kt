...
import com.dreweaster.ddd.kestrel.domain.CommandDispatcher
import com.dreweaster.ddd.kestrel.domain.DomainCommand
import com.dreweaster.ddd.kestrel.domain.DomainEvent
import com.dreweaster.ddd.kestrel.domain.DomainEventTag
import com.dreweaster.ddd.kestrel.domain.EventScheduler
import com.dreweaster.ddd.kestrel.domain.ProcessManager
import com.dreweaster.ddd.kestrel.domain.ProcessManagerContext
import com.dreweaster.ddd.kestrel.domain.ProcessManagerState
import com.dreweaster.ddd.kestrel.domain.ProcessManagerStepBuilder
import com.dreweaster.ddd.kestrel.domain.SuccessfullyExecutedStep
import com.dreweaster.ddd.kestrel.domain.Suspend
...
// Really important to note that any async actions, dispatched commands, emitted events will be triggered with
// at-least-once semantics
// If any of these fail, then they will all be re-executed. So, it's important to apply the idempotent receiver pattern
// Events emitted to self will be idempotent handled within a configured period*, and will be de-duped based on event
// id.
//
// * You Can configure how many event ids to retain in history. When not using snapshotting, this is irrelevant as all
// event ids will be retained. It comes in to play only with snapshots which can be configured to store a variable
// number of historic event ids
//
// It's not acceptable to run async DomainService functions outside an async action block. Behaviour is undefined if
// attempted.
//
...
// Events
sealed class BatchSessionsEvent : DomainEvent {
    override val tag = DomainEventTag("batch-parking-sessions-event")
}
                    ...
                    when {
                        state.isWithinBufferingPeriod(evt.finishedAt) ->
                            ...
                            val completedBatchId =
                                AggregateId() // TODO: create deterministic (but unique to this batch) ID
                            goto(
                                CreatingBatch(
                                    carParkId = state.carParkId,
                                    vehicle = state.vehicle,
...
class ProcessManagerEntryPoint<C : ProcessManagerContext, E : DomainEvent, S : ProcessManagerState>(
    ...
    suspend fun process() {
        ...
        when (val executedStep = builder.execute()) {
...
data class ProcessManagerScheduledEvent<C : ProcessManagerContext, E : DomainEvent, S : ProcessManagerState>(
...
interface DomainModel {
    ...
    fun <C : ProcessManagerContext, E : DomainEvent, S : ProcessManagerState> addProcessManagerListener(
        processManagerType: ProcessManager<C, E, S>,
        listener: ProcessManagerListener<C, E, S>,
    )
}
